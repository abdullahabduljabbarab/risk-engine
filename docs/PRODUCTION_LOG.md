...
- Keyless CI deploy via Workload Identity Federation: GitHub Actions mints an OIDC token and impersonates a dedicated least-privilege deploy service account through a GitHub provider scoped to the risk-engine repository, so no long-lived service-account key is stored in the repo. The ledger and orchestrator keep their key-based deploy for now; consolidating them onto this model is left to platform-infrastructure.
- Terraform for the engine's slice of the project: its database and user on the shared Cloud SQL instance, Artifact Registry, the database-url secret injected at runtime, a dedicated runner service account, the `risk-events` topic and its publisher grant, the Cloud Run service, a push subscription on the orchestrator's `payment-events` topic delivering to the consumer endpoint with a dead-letter topic and the Pub/Sub service-agent grants dead-lettering needs, and the Workload Identity pool, provider, deploy service account and repo-scoped binding. A `terraform` CI workflow (fmt, init, validate) matching the other services.
- Live bootstrap of the infrastructure: the Workload Identity pool, provider and deploy service account with its roles and the repo-scoped binding; the `risk` database and user on the shared instance; the `risk-database-url` secret with runtime access; the Artifact Registry repository; and the `risk-events` and dead-letter topics with the publisher grant.

### Problems / Decisions
- Keyless deploy over a stored key. Both options first considered came down to which long-lived JSON key to put in GitHub. Workload Identity Federation removes the key entirely: GitHub proves its identity with a short-lived OIDC token and GCP exchanges it to impersonate the deploy service account, with a repo-scoped condition so only this repository can. It is the better architecture and gives platform-infrastructure a real migration story later.
